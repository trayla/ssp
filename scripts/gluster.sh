...
function attach_datadisk () {
  ...
  ansible gluster$1 -i $BASEDIR/../ansible/inventory.yaml -a "pvcreate /dev/$DEVICE"
}
...
function create_datanode () {
  ...
  ansible gluster$1 -i $BASEDIR/../ansible/inventory.yaml -a "apt install -y lvm2 xfsprogs software-properties-common"

  # Attach the first data disks
  attach_datadisk $1 vdb 100G

  # Create a volume group
  ansible gluster$1 -i $BASEDIR/../ansible/inventory.yaml -a "vgcreate vg0 /dev/vdb"

  # Create a logical volume
  ansible gluster$1 -i $BASEDIR/../ansible/inventory.yaml -a "lvcreate -n lv0 -l 100%VG vg0"

  # Create a partition
  ansible gluster$1 -i $BASEDIR/../ansible/inventory.yaml -a "echo ';' | sfdisk /dev/vg0/lv0"

  # Format the disk
  ansible gluster$1 -i $BASEDIR/../ansible/inventory.yaml -a "mkfs.xfs /dev/vg0/lv0"

  # Create the data directory
  ansible gluster$1 -i $BASEDIR/../ansible/inventory.yaml -m file --args='path=/data state=directory mode=0755'

  # Mount the data partition
  ansible gluster$1 -i $BASEDIR/../ansible/inventory.yaml -m mount --args='path=/data src=/dev/vg0/lv0 fstype=xfs state=mounted'
}
...
function create_arbiternode () {
  ...
  # Create the virtual machine
  $BASEDIR/../scripts/deploy-vm.sh gluster$1 1024 2 20G pw $IPADDR
...
if [ "$1" == "install" ]; then
  ...
  ansible-playbook -i $BASEDIR/../ansible/inventory.yaml $BASEDIR/../ansible/gluster-hosts.yaml
  ...
  ansible gluster -i $BASEDIR/../ansible/inventory.yaml -m file --args="path=/data/$NS/$NAME state=directory mode=0755"
  ansible gluster0 -i $BASEDIR/../ansible/inventory.yaml -a "gluster volume create $NS-$NAME replica 2 arbiter 1 gluster1:/data/$NS/$NAME gluster2:/data/$NS/$NAME gluster0:/data/$NS/$NAME --mode=script"
  ansible gluster0 -i $BASEDIR/../ansible/inventory.yaml -a "gluster volume start $NS-$NAME --mode=script"

elif [ "$1" == "remove-volume" ]; then
  NS=$2
  NAME=$3

  ansible gluster0 -i $BASEDIR/../ansible/inventory.yaml -a "gluster volume stop $NS-$NAME --mode=script"
  ansible gluster0 -i $BASEDIR/../ansible/inventory.yaml -a "gluster volume delete $NS-$NAME --mode=script"
  ansible gluster -i $BASEDIR/../ansible/inventory.yaml -m file --args="path=/data/$NS/$NAME state=absent"
